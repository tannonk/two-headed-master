...
if [ $# != 3 ]; then
   echo "usage: $0 [--mode display|save] [--format pdf|svg] <utt-id> <lattice-ark> <trans_type>"
   echo "e.g.:  $0 utt-0001 \"test/lat.*.gz\""
   exit 1;
fi
...
outdir="/mnt/tannon/case_studies/"
# tmpdir=$(mktemp -d /tmp/kaldi.XXXX); # trap "rm -r $tmpdir" EXIT # cleanup
# tmpdir="/mnt/tannon/case_studies"

gunzip -c $lat | lattice-to-fst --lm-scale=$lm_scale --acoustic-scale=$acoustic_scale ark:- "scp,p:echo $uttid $outdir/$uttid.$transtype.fst|" || exit 1;
! [ -s $outdir/$uttid.$transtype.fst ] && \
