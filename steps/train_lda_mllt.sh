...
while [ $x -lt $num_iters ]; do
  ...
  if echo $mllt_iters | grep -w $x >/dev/null; then
    if [ $stage -le $x ]; then
      ...
      ## Fran: for some reason related to the NFS configuration, this call
      # fails pretty frequently when opening the output file
      ## Original call:
      # gmm-transform-means  $dir/$x.mat.new $dir/$x.mdl $dir/$x.mdl \
      #   2> $dir/log/transform_means.$x.log || exit 1;
      ## New
      gmm-transform-means  $dir/$x.mat.new $dir/$x.mdl $dir/$x.mdl.aux \
        2> $dir/log/transform_means.$x.log || exit 1;
      mv $dir/$x.mdl.aux $dir/$x.mdl
      #
      ## /Fran
