...
#
# Parameters:
# - Input csv: csv file, with the fields utt_id,transcription,speaker_id
#              duration,speech-in-speech,no-relevant-speech
...
transcriptions="$tmp_dir/transcriptions.txt"
...
$scripts_dir/process_archimob_csv.py -i $input_csv -p -t $transcriptions \
				     -s $spn_word -n $sil_word -o $wav_lst
...
cut -f 2 $transcriptions | perl -pe 's#(\s)+#\1#g' > $utterances
