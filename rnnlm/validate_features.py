...
parser = argparse.ArgumentParser(
    description="Validates features file, produced by rnnlm/choose_features.py.",
        ...
        if len(fields) == 3 and fields[1] == "length":
            ...
            if fields[1] == "constant":
                ...
                if float(fields[2]) <= 0.0:
                    sys.exit(
                        sys.argv[0]
                        + ": log-unigram-ppl should be a positive value: {0}".format(
                            fields[2]
                        )
                    )
                if has_unigram:
                ...
                if fields[2] in word_feats:
                    sys.exit(
                        sys.argv[0] + ": duplicated word feature: {0}".format(fields[2])
                    )
                ...
                if fields[2] in final_feats:
                    sys.exit(
                        sys.argv[0]
                        + ": duplicated final feature: {0}".format(fields[2])
                    )
                ...
                if fields[2] in match_feats:
                    sys.exit(
                        sys.argv[0]
                        + ": duplicated match feature: {0}".format(fields[2])
                    )
