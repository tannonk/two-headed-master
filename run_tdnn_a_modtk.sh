...
text=$1 # data/local/dict_nosp_larger/cleaned.gz
lexicon=$2 # data/local/dict_nosp_larger/lexiconp.txt
text_dir=$3 # output text dir (gets created) --> data/rnnlm/text_nosp
out_dir=$4 # output dir (gets created) --> exp/rnnlm_tdnn_a
...
if [ $stage -le 0 ]; then
  ...
  cat $text | awk -v text_dir=$text_dir '{if(NR%100 == 0) { print >text_dir"/dev.txt"; } else {print;}}' > $text_dir/train.txt
...
EOF
  ...
  ### choose features
  rnnlm/choose_features.py --unigram-probs=$out_dir/config/unigram_probs.txt \
