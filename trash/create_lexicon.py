...
def get_args():
    ...
    parser.add_argument('--cluster-file', '-c',
                        help='File with the consonant clusters', required=True)
    ...
    parser.add_argument('--map-diacritic', '-m',
...
def read_clusters(clusters_file, verbose=True):
    """
...
def main():
    ...
    if args.n2d:
        ...
        with open(args.n2d, 'r', encoding='utf8') as norm2dieth_file:
            ...
            for word in input_f:
                ...
                for form in set(dieth_forms):
                    ...
                    # print(transcription)
                    for pron in transcription:
                        # avoid duplicates in lexicon!
                        if (word, pron) not in seen_pairs:
                            output_f.write('{0} {1}\n'.format(word, pron))
                            seen_pairs.add((word, pron))
        ...
        for word in input_f:
            ...
            transcription = transcribe_simple(
                word.lower(), clusters, max_length_cluster, args.map_diacritic)
